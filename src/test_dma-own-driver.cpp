...
#define LENGTH 100
#define LENGTH_bits LENGTH * sizeof(uint32_t)
...
int main() {
  ...
  int status = dma.SimpleTransfer((UINTPTR)(P_START + LENGTH_bits), LENGTH_bits,
  ...
  if (dma.SimpleTransfer((UINTPTR)(P_START), LENGTH_bits, DMA_DIR_DMA_TO_DEVICE) !=
      0) {
  ...
  pmem.gather(rx_buff, LENGTH, LENGTH_bits);
